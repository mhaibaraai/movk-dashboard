...
- **读操作提示策略**：列表（`useApiFetch`）与 `$api` 的 `tree`、`children-ids` 等查询端点传 `{ toast: false }` 避免弹提示；`getById` 详情端点用默认提示（不传 `toast: false`），写操作（增删改）保留默认提示。
...
export function usePostApi() {
  ...
  return {
    ...
    getById: (id: string) => $api<PostResp>(`/v1/system/posts/${id}`)
  }
}
```
